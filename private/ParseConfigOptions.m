...
% ParseConfigOptions is executed by MVCTdose to open the config file
...
Event(['Default file path set to ', config.path]);

% Separate out pitch options and values
config.PITCH_OPTIONS = strsplit(config.PITCH_OPTIONS, ',');
config.PITCH_VALUES = str2double(strsplit(config.PITCH_VALUES, ','));

% Parse default period as a double
config.DEFAULT_PERIOD = str2double(config.DEFAULT_PERIOD);
Event(sprintf('Default gantry period set to %0.1f sec', ...
    config.DEFAULT_PERIOD));

% Parse sadose option
config.USE_SADOSE = str2double(config.USE_SADOSE);
Event(sprintf('SADOSE set to %i', config.USE_SADOSE));
